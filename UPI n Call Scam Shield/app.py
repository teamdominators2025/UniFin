...
def check_upi():
    ...
    if scam_triggers or is_suspicious_id:
        trigger_list = ", ".join(scam_triggers) if scam_triggers else "Suspicious ID"
        return jsonify({
            "level": "high",
            "result": f"🚨 FRAUD TRIGGER: Found scam indicators: {trigger_list}. Real bank staff will NEVER ask you to 'Approve' or 'Refund' via UPI PIN."
        })
