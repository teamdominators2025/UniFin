...
/* ===============================
   UNIFIN — CLIENT SIDE SCAM LOGIC
   Converted from Flask backend
================================ */

const SCAM_KEYWORDS = [
  "pin", "cashback", "refund", "lottery",
  "prize", "approve", "winner", "reward"
];

const PHISHING_DOMAINS = [
  "bit.ly", "tinyurl.com", "t.co",
  "kyc-update", "rbi-safety",
  "gpay-reward", "customer-care-helpline"
];
...
function checkURL() {
  ...
  if (isPhishing || isInsecure) {
    res.innerHTML =
      "🚨 CRITICAL: This link is suspicious. It uses insecure HTTP or a known phishing domain.";
    res.style.color = "red";
  } else {
    res.innerHTML =
      "✅ URL appears safe, but always verify the sender before clicking.";
...
function checkUPI() {
  ...
  if (!upiId && !message) {
    res.innerHTML = "❌ Please enter UPI ID or message.";
    res.style.color = "orange";
    return;
  }
  ...
  const suspiciousId =
    upiId.includes("refund") ||
    upiId.includes("support") ||
    upiId.includes("care") ||
    upiId.includes("helpline");

  if (triggers.length > 0 || suspiciousId) {
    res.innerHTML =
      "🚨 FRAUD TRIGGER: Scam indicators detected — never share UPI PIN or approve unknown requests.";
    res.style.color = "red";
  } else {
    res.innerHTML =
      "✅ No obvious fraud indicators found.";
    res.style.color = "green";
  }
}
...
function scanQR(event) {
  ...
  reader.onload = () => {
    ...
    img.onload = () => {
      ...
      if (!code) {
        res.innerHTML = "❌ No QR code detected.";
        res.style.color = "orange";
        return;
      }
      ...
      if (content.includes("upi://pay")) {
        res.innerHTML =
          "🚨 PAYMENT TRAP: This QR will SEND money, not receive it.";
        res.style.color = "red";
      } else {
        res.innerHTML =
          "✅ QR does not contain a direct UPI payment request.";
        res.style.color = "green";
      }
    };
