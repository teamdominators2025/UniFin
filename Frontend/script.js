...
/* =========================================
   3D IMMERSIVE BACKGROUND & SCROLLYTELLING
   ========================================= */
...
function initParallax() {
    ...
    window.addEventListener('scroll', () => {
        ...
        shapes.forEach(shape => {
            if (shape.el) {
                // Move element based on scroll position and defined speed
...
// --- 2. THREE.JS 3D NETWORK ANIMATION ---
function initThreeJS() {
    ...
    // B. Create the "Fintech Data Network" (Particles & Lines)
    ...
    // Material for the dots (nodes)
    const material = new THREE.PointsMaterial({
        color: 0x6366f1, // Matches your CSS var(--accent)
        size: 0.4,
        transparent: true,
        opacity: 0.8
    });
    ...
    // Add subtle interaction on mouse move
    ...
    const animate = function () {
        ...
        // 2. Scrollytelling Interaction
        // Rotate the 3D object based on scroll position
        const scrollY = window.scrollY;
        particles.rotation.x = scrollY * 0.0005; // Rotates vertically as you scroll down
...
// REPLACE your existing initThreeJS function with this one for testing:

function initThreeJS() {
    const container = document.getElementById('three-js-background');
    if (!container) {
        console.error("3D Container not found! Make sure <div id='three-js-background'></div> is in your HTML.");
        return;
    }

    console.log("Initializing 3D Scene..."); // Check console for this message
    ...
    // DEBUG MATERIAL: Big and Red
    const material = new THREE.PointsMaterial({
        color: 0xff0000, // Bright RED for debugging
        size: 2.0,       // Huge size to ensure visibility
        transparent: true,
        opacity: 1.0
    });
    ...
    const animate = function () {
        requestAnimationFrame(animate);
        particles.rotation.y += 0.005; // Faster rotation
...
function initThreeJS() {
    ...
    // 2. Camera (Wide angle for depth)
    const camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
    camera.position.z = 50; // Pull camera back to see the whole cloud
    ...
    // 4. Create the "Soft Circle" Texture programmatically
    // This removes the "square" look without needing an external image file
    const getTexture = () => {
        ...
        // Draw a soft radial gradient (white center, transparent edge)
    ...
    // 5. Create the Particle Cloud
    const particleCount = 600; // High count for a dense "data" feel
    const geometry = new THREE.BufferGeometry();
    const positions = [];

    // Distribute particles in a wide sphere
    for (let i = 0; i < particleCount; i++) {
    ...
    // Elegant Material Settings
    const material = new THREE.PointsMaterial({
        color: 0x6366f1,    // Matches your Indigo Accent
        size: 1.5,          // Small and subtle
        map: getTexture(),  // Use the soft circle texture
        transparent: true,
        opacity: 0.6,       // Slight transparency
        depthWrite: false,  // Prevents glitchy overlapping
        blending: THREE.AdditiveBlending // Makes them glow when they overlap
    });

    const particleSystem = new THREE.Points(geometry, material);
    scene.add(particleSystem);

    // 6. Animation Loop (The Scrollytelling Part)
    let mouseX = 0;
    let mouseY = 0;
    let targetX = 0;
    let targetY = 0;
    ...
    const animate = function () {
        ...
        // Scrollytelling: Rotate based on scroll position
        const scrollY = window.scrollY;
        
        // Gentle constant rotation
        particleSystem.rotation.y += 0.001;
        
        // Dynamic rotation based on scroll (The "Parallax" feel)
        particleSystem.rotation.x += 0.0005 + (mouseY * 0.05);
        
        // Move the whole cloud slightly up/down with scroll
...
const translations = {
    en: {
        ...
        desc_budget: "AI-driven tools to track, analyze, and optimize your monthly spending.",
...
// Initialize on load to restore saved language
document.addEventListener('DOMContentLoaded', () => {
